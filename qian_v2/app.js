...
function pickOne() {
  ...
  if (!pool.length) pool = QIAN; // 一周7卡对506池不可能耗尽，防御兜底
...
function updateWeek() {
  ...
  $("weekInfo").textContent = names.length ? "本周已学：" + names.join(" · ") : "";
}
...
function drawShareCard(fmt) {
  ...
  ctx.fillText("每天一句 · 学而时习之", W / 2, by + px(56));
...
$("dlBtn").onclick = () => {
  ...
  a.download = "时习论语日课_" + todayStr() + (shareState.fmt === "v" ? "_竖版" : "_方形") + ".png";
