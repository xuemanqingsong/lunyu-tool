// 时习 · 论语摇一摇全量版 —— 检索层 + 页面逻辑
// 与单词版（/one/）共用同一套锚点映射与打分算法；
// 数据为《论语》全量 506 章（含 insight 引申讲解 / practice 今日实践，卡片中默认折叠）。
// API 写入 /full/data/，与单词版完全隔离。
...
function detailBlock(ch) {
  ...
  const practices = ch.practice || [];
  if (!insight && practices.length === 0) return "";
  return `<details class="detail">
      <summary>展开讲解 · 今日一试</summary>
      ${insight ? `<div class="insight">${esc(insight)}</div>` : ""}
      ${practices.length ? `
      <div class="practice-title">今日一试</div>
      <div class="practices">
        ${practices.map(p => `<div class="practice">${esc(p)}</div>`).join("")}
      </div>` : ""}
    </details>`;
}
...
今日一试：
${(ch.practice || []).slice(0, 2).map(p => `· ${p}`).join("\n")}`;
